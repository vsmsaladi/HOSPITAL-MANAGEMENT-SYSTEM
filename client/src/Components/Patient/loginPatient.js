	...
	async handleSubmit(childEmail, childPswrd) {
		...
		axios.post("http://localhost:12347/login", this.state).then((res) => {
			...
			if (res.data.token) {
				...
				// sessionStorage.setItem("patientName", jwtDecode(res.data.token).name);
				Cookies.set("patientName", jwtDecode(res.data.token).name);
				// sessionStorage.setItem("patient_id", jwtDecode(res.data.token).Id);
				Cookies.set("patient_id", jwtDecode(res.data.token).Id);
