...
class GetAppointments extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            appointments: [], // Initialized as an empty array to avoid null checks
            searchTerm: "",
            isModalOpen: false,
            selectedAppointment: null,
            prescriptions: [],
        ...
        axios
            ...
            .then((res) => {
                ...
                if (res.data && Array.isArray(res.data)) {
                    ...
                    console.log("Invalid or empty response:", res.data);
                }
            })
            .catch((error) => {
                ...
                <Row>
                    ...
                    <Col className="mt-3">
                        ...
                        >
                            ...
                            <tbody>
                                {Array.isArray(this.state.appointments) && this.state.appointments.length > 0 ? (
                                    this.state.appointments
                                        .filter((appointment) => {
                                            if (this.state.searchTerm === "") {
                                                return appointment;
                                            } else if (
                                                appointment.Name.toLowerCase().includes(this.state.searchTerm.toLowerCase())
                                            ) {
                                                return appointment;
                                            }
                                        })
                        ...
                        {Array.isArray(this.state.prescriptions) && this.state.prescriptions.length > 0 ? (
                            <div>
                                ...
                                <ul>
                                    {this.state.prescriptions.map((prescription, index) => (
                                        <li key={index}>
                                            {prescription.prescription_text} (Date: {prescription.prescription_date})
                                        </li>
                                    ))}
                                </ul>
                            </div>
