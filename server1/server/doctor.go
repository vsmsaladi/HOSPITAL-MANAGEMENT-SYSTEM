...
// 	rows, err := db.Query("SELECT prescription_id, apid, prescription_text, prescription_date, doctor_id FROM prescriptions WHERE apid = ?", temp.Apid)
// 	if err != nil {
...
// 		if err := rows.Scan(&pres.PrescriptionID, &pres.Apid, &pres.Prescription, &pres.PrescriptionDate, &pres.DoctorID); err != nil {
